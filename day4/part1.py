...
#find the guard asleep the most minutes
sleepiest_guard = [0,0]
for guard_id in sleep_activity:
    guard_minutes_asleep = 0
    for minute_activity in sleep_activity[guard_id]:
        guard_minutes_asleep += sleep_activity[guard_id][minute_activity]
    if guard_minutes_asleep > sleepiest_guard[1]:
        sleepiest_guard = [int(guard_id),int(guard_minutes_asleep)]

#find the minute the sleepiest guard was asleep the most
sleepiest_minute = [0,0]
sleepiest_guard = int(sleepiest_guard[0])

for minute_activity in sleep_activity[str(sleepiest_guard)]:
    if sleep_activity[str(sleepiest_guard)][minute_activity] > sleepiest_minute[1]:
        sleepiest_minute = [int(minute_activity),int(sleep_activity[str(sleepiest_guard)][minute_activity])]
